...
class FourthStarterScreen extends StatefulWidget {
  const FourthStarterScreen({super.key});

  @override
  State<FourthStarterScreen> createState() => _FourthStarterScreenState();
}

class _FourthStarterScreenState extends State<FourthStarterScreen> with SingleTickerProviderStateMixin {
        ...
        children: [
          ...
          Center(
            child: AnimatedBuilder(
              animation: _animationController,
              builder: (context, child) {
                // Calcul de l'opacité de la bordure (diminue pendant le début de l'animation puis revient)
                double borderOpacity = 0.4;
                if (_animationController.value < 0.2) {
                  // Diminue pendant les premiers 20% de l'animation
                  borderOpacity = 0.4 * (1 - (_animationController.value * 5));
                } else if (_animationController.value > 0.8) {
                  // Revient progressivement pendant les derniers 20% de l'animation
                  borderOpacity = 0.4 * (((_animationController.value - 0.8) * 5));
                }
